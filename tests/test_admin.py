...
def test_list_sessions_all():
    sessions = admin.list_sessions(get_client("Sessions"), None)
    assert rpc_called("Sessions", "ListSessions", 1)
    assert sessions == []

def test_list_sessions_running():
    sessions = admin.list_sessions(get_client("Sessions"), SessionFieldFilter.STATUS == SessionStatus.RUNNING)
    assert rpc_called("Sessions", "ListSessions", 2)
    assert sessions == []

def test_list_sessions_cancelled():
    sessions = admin.list_sessions(get_client("Sessions"), SessionFieldFilter.STATUS == SessionStatus.CANCELLED)
    assert rpc_called("Sessions", "ListSessions", 3)
    assert sessions == []

def test_cancel_single_session():
    admin.cancel_sessions(get_client("Sessions"), ["session-id"] )
    assert rpc_called("Sessions", "CancelSession")

def test_cancel_multiple_sessions():
    admin.cancel_sessions(get_client("Sessions"), ["session-id1","session-id2","session-id3"] )
    assert rpc_called("Sessions", "CancelSession", 4)

def test_list_tasks_all():
    tasks = admin.list_tasks(get_client("Tasks"), admin.create_task_filter("session-id", True, False, False) )
...
def test_check_single_task():
    task = admin.check_tasks(get_client("Tasks"), ["task-id"])
    assert rpc_called("Tasks", "GetTask")
    assert task[0].id == "task-id"

def test_check_multiple_task():
    task = admin.check_tasks(get_client("Tasks"), ["task-id", "task-id", "task-id"])
...
def test_check_non_existing_task():
    task = admin.check_tasks(get_client("Tasks"), ["non-existing-id"])
