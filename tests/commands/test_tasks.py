...
@pytest.mark.parametrize("cmd", [f"task list id -e {ENDPOINT}"])
...
@pytest.mark.parametrize(
    "cmd",
    [
        f"task get --endpoint {ENDPOINT} {serialized_tasks[0]['Id']}",
    ],
)
def test_task_get(mocker, cmd):
    mocker.patch.object(ArmoniKTasks, "get_task", return_value=raw_tasks[0])
    result = run_cmd_and_assert_exit_code(cmd)
    assert reformat_cmd_output(result.output, deserialize=True) == [serialized_tasks[0]]


@pytest.mark.parametrize("cmd", [f"task cancel --endpoint {ENDPOINT} {serialized_tasks[0]['Id']}"])
...
def test_task_create(mocker, cmd):
