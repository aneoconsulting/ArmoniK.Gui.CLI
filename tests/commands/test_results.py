...
@pytest.mark.parametrize("cmd", [f"result list id -e {ENDPOINT}"])
...
@pytest.mark.parametrize(
    ...
    [
        (
            f"result delete_data --endpoint {ENDPOINT} {serialized_results[0]['ResultId']} --yes --debug",
            {serialized_results[0]["SessionId"]: [serialized_results[0]["ResultId"]]},
        ),
        (
            f"result delete_data --endpoint {ENDPOINT} {serialized_results[0]['ResultId']} {serialized_results[1]['ResultId']} --yes --debug",
            {
...
def test_result_upload_data_file_not_found(mocker):
    ...
    result = run_cmd_and_assert_exit_code(cmd, split=False, exit_code=1)

    assert "Could not open file" in str(result.output)
